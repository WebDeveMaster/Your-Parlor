...
async function renderSalons(signal) {
    ...
    try {
        ...
        salons.forEach((salon, index) => {
            ...
            card.innerHTML = `
            ${salon.WholeServiceDiscounting > 0 ? `<h3 style="height: 70px; text-align: center; padding-top: 6px; margin-top: 0px; margin-bottom: -30px; background-color:rgb(255, 255, 255); border-radius: 10px 0px; font-size: 100%; color:rgb(255, 69, 69);
                                                                    position: sticky;
                                                                    top: 50px;
                                                                    z-index: 700;">Discount ${salon.WholeServiceDiscounting} pkr saab services mai</h3>` : ""}
                <div class="slider" data-slider-id="${sliderId}" style="
                                                                    position: sticky;
                                                                    top: 52px;
                                                                    bottom: -20%;
                                                                    z-index: 800;
                ...
                <div class="details">
                    ...
                    <p style="font-size: 82%; margin-top: -8px;"><strong style="color: black;">Owner:</strong> ${salon.ownerName}</p>
                    <p style="font-size: 82%; margin-top: -10px;"><strong style="color: black;">Opened:</strong> ${`${convertTo12HourFormat(salon.openTime) || 'N/A'} - ${convertTo12HourFormat(salon.closeTime) || 'N/A'}`}</p>
                    <p class="location" style="margin-left: 1px; margin-bottom: -5px; margin-top: -5px; line-height: 17px;"><strong style="font-size: 90%; color: black;">Location:</strong> <a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(salon.location)}" target="_blank" style="font-size: 90%;  color: rgb(0, 157, 255);">${salon.location}</a></p>
                    <div style="height: 8px;"> </div>
                    <button style="margin: 0px; margin-top: 10px;" class="btn" onclick="debounceShowSection('user-booking', '${salon.salonName}', '${salon.ownerName}', '${salon.location}')">Book Now</button>
                    </div>
                    `;
...
/* ===== main render ===== */
function renderBookings(bookings, gridId='bookingsGrid', sortAsc=true, lsKey='cachedBookings'){
  ...
  bookings.sort((a,b)=>{
    ...
    if(dA-dB!==0) return sortAsc ? dA-dB : dB-dA;
    return sortAsc
  ...
  bookings.forEach((b,idx)=>{
    ...
    /* top line: “Booking X” + canceled badge (optional) */
    card.innerHTML = `
      <div class="booking-header">
        <span class="label" style="padding-left: 40%;" >Booking ${idx+1}</span>
        ${b.status==='user canceled' ? `<span class="status-badge">By Customer</span>` : ''}
      </div>

      <div class="booking-grid-inner">
        <div><span class="label">Name:</span> ${b.customerName}</div>
        <div><span class="label">Service:</span> ${b.service}</div>
        <div><span class="label">Price:</span> ${b.price}</div>
        <div><span class="label">Date:</span> ${formatDate(b.date)}</div>
        <div><span class="label">Time:</span> ${b.time}</div>
        <div><span class="label">Seat #:</span> ${b.time.substring(b.time.indexOf('s')+1)}</div>
        <div><span class="label">Code:</span> ${b.code}</div>
      </div>

      ${b.status==='pending'
        ? `<div>
             <button class="btn" onclick="dash_cancelBooking('${b.code}')">Cancel</button>
             <button class="btn" onclick="dash_Complete_Customer('${b.code}')">Complete</button>
           </div>`
        : ''}`;
    grid.appendChild(card);
  });

  /* —— optional: cache list in localStorage —— */
  localStorage.setItem(lsKey, JSON.stringify(bookings.map(b=>b.code)));
}
